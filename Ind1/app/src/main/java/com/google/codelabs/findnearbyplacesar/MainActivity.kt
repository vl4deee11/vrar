...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun addPlaces(anchorNode: AnchorNode) {
        ...
        for (place in places) {
            ...
            placeNode.localPosition =
                place.getPositionVector(y, orientationAngles[0], currentLocation.latLng)
        }
    }
    ...
    private fun getCurrentLocation(cb: (Location) -> Unit) {
    ...
    private fun setPlaces(location: Location) {
        ...
        this.places = listOf(
            Place(
                id = "test",
                name = "Tag at lat=${loc.lat},lon=${loc.lng}, point=${Random.nextInt()}",
                geometry = Geometry(
                    location = loc
                )
            )
